package io.github.opencubicchunks.cubicchunks.figureoutwheretoputthislater;
...
// TODO move this to cc_core?
public class UserFunction {
    public static final Codec<UserFunction> CODEC = RecordCodecBuilder.create(
            (instance) -> instance.group(Codec.list(Entry.CODEC).fieldOf("values").forGetter((UserFunction config) -> Arrays.asList(config.values)))
    ...
    public UserFunction(Map<Float, Float> funcMap) {
        values = funcMap.entrySet().stream().sorted(Map.Entry.comparingByKey()).map(e -> new Entry(e.getKey(), e.getValue())).toArray(Entry[]::new);
    }
    ...
    public static class Entry {
        public static final Codec<Entry> CODEC = RecordCodecBuilder.create((instance) -> instance
                .group(Codec.FLOAT.fieldOf("y").forGetter((Entry config) -> config.y), Codec.FLOAT.fieldOf("v").forGetter((Entry config) -> config.v))
